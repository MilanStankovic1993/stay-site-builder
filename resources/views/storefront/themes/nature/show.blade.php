...
<body class="bg-[var(--site-sand)] text-slate-900">
    ...
    <div class="relative overflow-hidden">
        <div class="absolute inset-x-0 top-0 h-[30rem] bg-[radial-gradient(circle_at_top,_rgba(181,137,77,0.18),_transparent_34%)]"></div>
            ...
            <div class="mx-auto max-w-7xl px-6 pb-16 pt-8 sm:px-8 lg:px-10 lg:pb-24">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        @endif
                        <a href="{{ route('home') }}" class="text-xs uppercase tracking-[0.4em] text-[var(--site-primary)]">StaySite Builder</a>
                    </div>
                    <div class="rounded-full bg-white/80 px-4 py-2 text-xs uppercase tracking-[0.28em] text-[var(--site-secondary)] shadow-[0_12px_35px_rgba(53,83,69,0.08)]">
                        Nature theme
                    </div>
                </div>
                ...
                <div class="mt-12 grid gap-8 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
                    <div>
                        ...
                        <p class="mt-6 max-w-2xl text-lg leading-8 text-slate-700">
                            {{ $siteShortDescription ?: 'Topao i savremen sajt za smestaje koji prodaju prirodu, mir i dozivljaj boravka.' }}
                        </p>

                        <div class="mt-8 flex flex-wrap gap-3">
                            <a href="#upit" class="rounded-full bg-[var(--site-primary)] px-6 py-3 text-sm font-semibold text-white shadow-[0_18px_45px_rgba(53,83,69,0.18)]">
                                {{ __('site.storefront.send_inquiry') }}
                            </a>
                            @if ($accommodation->whatsapp_number)
                                <a href="https://wa.me/{{ preg_replace('/\D+/', '', $accommodation->whatsapp_number) }}" class="rounded-full border border-[var(--site-primary)]/20 bg-white px-6 py-3 text-sm font-semibold text-[var(--site-primary)]">
                                    WhatsApp
                                </a>
                            @endif
                        </div>
                        ...
                        <div class="mt-8 flex flex-wrap gap-3">
                            <div class="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-[0_12px_35px_rgba(53,83,69,0.08)]">{{ $accommodation->max_guests ?: '-' }} gostiju</div>
                            <div class="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-[0_12px_35px_rgba(53,83,69,0.08)]">{{ $accommodation->bedrooms ?: '-' }} sobe</div>
                            <div class="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-[0_12px_35px_rgba(53,83,69,0.08)]">{{ $accommodation->bathrooms ?: '-' }} kupatila</div>
                            <div class="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-[0_12px_35px_rgba(53,83,69,0.08)]">{{ $accommodation->price_from ? number_format((float) $accommodation->price_from, 0, ',', '.') . ' ' . $accommodation->currency : 'Na upit' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2 overflow-hidden rounded-[2.2rem] shadow-[0_25px_80px_rgba(53,83,69,0.14)]">
                            ...
                            @else
                                <div class="flex h-80 items-center justify-center bg-[var(--site-mist)] text-[var(--site-primary)]">Hero fotografija uskoro</div>
                            @endif
                        </div>
                        @forelse ($accommodation->getMedia('gallery')->take(2) as $media)
                        ...
                        @empty
                            <div class="sm:col-span-2 rounded-[1.8rem] bg-[var(--site-mist)] p-8 text-[var(--site-primary)]">
                                Galerija ce uskoro biti dopunjena fotografijama smestaja.
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <main class="mx-auto max-w-7xl px-6 pb-24 sm:px-8 lg:px-10">
            <section class="grid gap-6 lg:grid-cols-[1fr_0.82fr]">
                <div class="rounded-[2rem] bg-white p-8 shadow-[0_25px_80px_rgba(53,83,69,0.08)]">
                ...
                <div class="rounded-[2rem] bg-[var(--site-mist)] p-8">
                    ...
                    <div class="mt-5 flex flex-wrap gap-3">
                        ...
                        @empty
                            <span class="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-500">{{ __('site.storefront.amenities_soon') }}</span>
                        @endforelse
                    </div>

                    <div class="mt-8 rounded-[1.8rem] bg-white p-6 shadow-[0_15px_40px_rgba(53,83,69,0.06)]">
                        <p class="text-sm uppercase tracking-[0.35em] text-[var(--site-secondary)]">{{ __('site.storefront.gallery') }}</p>
                        <p class="mt-3 text-base leading-7 text-slate-700">
                            Ova tema stavlja fokus na fotografije, prirodne tonove i osecaj odmora bez preopterecenog rasporeda.
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section class="mt-10 grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
                ...
                <div id="upit" class="rounded-[2rem] bg-white p-8 shadow-[0_25px_80px_rgba(53,83,69,0.08)]">
                            ...
                            @if ($externalLinks->isNotEmpty())
                                <div class="mt-6 flex flex-wrap gap-2">
                                    @foreach ($externalLinks as $link)
                                        <a href="{{ $link['url'] }}" target="_blank" rel="noreferrer" class="rounded-full border border-stone-200 bg-stone-50 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-700">
                                            {{ $link['label'] }}
                                        </a>
                                    @endforeach
                                </div>
